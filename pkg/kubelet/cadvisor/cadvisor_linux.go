...
const globalHousekeepingInterval = 5 * time.Second
const maxHousekeepingInterval = 2 * time.Second
const defaultHousekeepingInterval = 2 * time.Second
const allowDynamicHousekeeping = false

func init() {
	// Override cAdvisor flag defaults.
	flagOverrides := map[string]string{
		"global_housekeeping_interval": globalHousekeepingInterval.String(),
